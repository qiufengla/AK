...
/*
题目考点分析
这道题目的核心考点是图的连通性和连通分量计算，具体涉及以下关键知识点：
...
使用并查集（Union-Find）高效管理连通关系

或使用BFS/DFS进行图遍历统计连通分量

两种方法的时间复杂度均为O(nα(n))或O(n)

问题转换能力：

将"最小修复公路长度"问题转化为"连通分量数量减1"

理解在连通分量间需要修复的公路数量 = 连通分量数 - 1

边界条件处理：

传送目标城市必须存在（1 ≤ i±aᵢ ≤ n）

处理大规模输入数据（n ≤ 3×10⁵，总n ≤ 10⁶）
...
理解传送是双向的（i→i±aᵢ 和 i±aᵢ→i 是等价的）

连通性分析：

识别出修复公路的作用就是连接不同的连通分量

得出关键结论：最小修复长度 = 连通分量数 - 1
...
是典型的图论基础题，也是许多算法竞赛和面试中的常见题型，能有效检验候选人对基本算法数据结构的理解和应用能力。
